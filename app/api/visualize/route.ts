...
// ── Реализм клинкера ──────────────────────────────────────────────
// Добавляется к инструкциям МАТЕРИАЛА (не к structural lock). Лечит две
// проблемы: слишком крупный «мультяшный» кирпич и плоский нарисованный вид.
const CLINKER_SCALE =
  ` Render the clinker brick at REALISTIC scale: thin, elongated bricks. A standard clinker ` +
  `brick is ~240mm long × ~65mm high. Fit MANY rows across each wall storey — a typical house ` +
  `wall shows roughly 10-14 brick rows per floor, NOT 4-5 oversized blocks. Bricks must look ` +
  `proportionally small relative to windows and doors. Match brick size to real-world ` +
  `proportions using the windows and doors in IMAGE 1 as scale reference.`;
...
export async function POST(req: NextRequest) {
  ...
  // Реализм ЦОКОЛЯ: тот же принцип, что для стены (тонкий кирпич, фотореализм).
  if (plinthShapeAsset) {
    prompt += CLINKER_SCALE + (plinthColorAsset ? CLINKER_TEXTURE_B : "") + CLINKER_PHOTOREAL;
  } else if (plinthColorAsset) {
    prompt += CLINKER_PHOTOREAL;
  }
